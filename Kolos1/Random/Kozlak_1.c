...
do_cmd(char *cmd,int instruction, int final)      /* wykonaj program, policz jego czasy, sprawdz, czy to nie jest ostatni program */
{
   ...
   if (times(&tmsstart) == -1)           /* zapamietaj wartosci poczatkowe pomiaru czasu*/
    {
          printf("times error"); 
          exit(-1);
    }

    start = tmsstart.tms_utime;
   ...
   if (times(&tmsend) == -1)      /* zapamietaj wartosci koncowe */
      {printf("times error"); exit(-1);}

    end = tmsend.tms_utime;
